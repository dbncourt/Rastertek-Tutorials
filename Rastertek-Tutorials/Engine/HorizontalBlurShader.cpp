...
bool HorizontalBlurShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = HorizontalBlurShader::InitializeShader(device, hwnd, L"HorizontalBlurVertexShader.hlsl", L"HorizontalBlurPixelShader.hlsl");
	if (!result)
	{
		return false;
	}
...
void HorizontalBlurShader::Shutdown()
{
	...
	HorizontalBlurShader::ShutdownShader();
}

bool HorizontalBlurShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float screenWidth)
{
	bool result;

	//Set the shader parameters that it will use for rendering
	result = HorizontalBlurShader::SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, screenWidth);
	if (!result)
	{
		return false;
	}
	...
	HorizontalBlurShader::RenderShader(deviceContext, indexCount);

	return true;
}

bool HorizontalBlurShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vertexShaderFileName, WCHAR* pixelShaderFileName)
{
	HRESULT result;
	...
	if (FAILED(result))
	{
		...
		if (errorMessage)
		{
			HorizontalBlurShader::OutputShaderErrorMessage(errorMessage, hwnd, vertexShaderFileName);
		}
	...
	if (FAILED(result))
	{
		...
		if (errorMessage)
		{
			HorizontalBlurShader::OutputShaderErrorMessage(errorMessage, hwnd, pixelShaderFileName);
		}
...
bool HorizontalBlurShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float screenWidth)
{
	HRESULT result;
